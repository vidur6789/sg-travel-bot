...
def trippingoCarouselItem(item):    ####### number of items
    name = item['name']
    #print('length of recom:   ',len(name))
...
def getRecommendations(req):
    ...
    #print('\ngetRecommendation',carouselItems)
...
def trippingoRecommendations(req):
    ...
    #print("Get Recommendations for {}, {}, {}. ".format(travellerType, keywords, slots["location"]))
    #print(url)
    zipcode = getZipCode()
    url = TRIPPINGO_URL+"/recommendations?keywords={keywords}&travellerType={travellerType}&count=3".format(keywords=keywords,travellerType=travellerType);
    #print(url)
...
def googleRecommendations(req):
    #names = ["Gardens By the Bay", "Botanic Gardens", "Marina Bay"]
    category = getCombinedKeywords()
    names = getNearbyRecom(category)
    #print('get recommendation',names)
...
def webhook():
    ...
    if intent_name in REC_INTENTS:
        ...
        resp_dict = fulfillmentTextResponse("Sorry, I cannot understand you")
